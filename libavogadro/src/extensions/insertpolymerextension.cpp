...
namespace Avogadro {
  ...
  void InsertPolymerExtension::updateGraphics(QString smiles, QWebView *target)
  {
    ...
    //    OBOp* pOp = OBOp::FindType("genalias"); // use aliases when available

    // Do not draw "CH3" end-groups
    conv.AddOption("C",OBConversion::OUTOPTIONS);
    // Use asymmetric double-bonds
    conv.AddOption("s",OBConversion::OUTOPTIONS);
    // Use aliases if available (e.g., CO2H)
